...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {                        
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));               // make exclusive               
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*childNode) -> AddEdgeToParentNode(edge.get());            // make sure I put in proper place
    ...
    // add chatbot to graph root node                                                          // ********************* Task 5. *****************************
    ChatBot t5ChatBot = ChatBot("../images/chatbot.png");                                      // new instance
    t5ChatBot.SetChatLogicHandle(this);                                                        // set handle
    t5ChatBot.SetRootNode(rootNode);                                                           // set rootNode
    rootNode -> MoveChatbotHere(std::move(t5ChatBot));                                         // move it
}
